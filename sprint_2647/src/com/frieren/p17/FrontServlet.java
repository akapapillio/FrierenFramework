    ...
    private void initializeFramework() throws ServletException {
        try {
            ...
            for (Class<?> componentClass : scanner.getComponents()) {
                ...
                System.out.println("Singleton créé: " + componentClass.getName());
            }
            ...
            for (Class<?> controllerClass : scanner.getControllers()) {
                ...
                for (Method method : controllerClass.getDeclaredMethods()) {
                    ...
                    if (method.isAnnotationPresent(GetMapping.class)) {
                        urlPath = method.getAnnotation(GetMapping.class).value();
                        registerUrl(baseUrl + urlPath, "GET", method, controllerInstance);
                    }
                    if (method.isAnnotationPresent(PostMapping.class)) {
                        urlPath = method.getAnnotation(PostMapping.class).value();
                        registerUrl(baseUrl + urlPath, "POST", method, controllerInstance);
                    }
                    if (method.isAnnotationPresent(MyMap.class)) {
                        urlPath = method.getAnnotation(MyMap.class).url();
                        registerUrl(baseUrl + urlPath, "GET", method, controllerInstance);
                        registerUrl(baseUrl + urlPath, "POST", method, controllerInstance);
                    }
                }
            }
            System.out.println("🎯 Framework initialisé avec " + singletons.size() + " singletons et " + methodInstances.size() + " méthodes de contrôleurs.");
        } catch (Exception e) {
        ...
        for (Field field : targetInstance.getClass().getDeclaredFields()) {
            if (field.isAnnotationPresent(Autowired.class)) {
                ...
                if (dependency != null) {
                    ...
                    System.out.println("⚡ Dépendance injectée: " + fieldType.getSimpleName() + " dans " + targetInstance.getClass().getSimpleName());
                } else {
                    System.err.println("⚠️ Dépendance non trouvée pour " + fieldType.getName() + " dans " + targetInstance.getClass().getName());
                }
            }
        }
    }
